...
openfile_lookup(envid_t envid, uint32_t fileid, struct OpenFile **po)
{
	struct OpenFile *o;
       ...
       //cprintf("Mani lookup called for %x \n",fileid);

	o = &opentab[fileid % MAXOPEN];
        //cprintf("MANI ref %d fileid %x\n",pageref(o->o_fd),o->o_fileid);
...
try_open:
	...
	if (debug)
		cprintf("sending success, page %08x fileid %d\n", (uintptr_t) o->o_fd,o->o_fd->fd_file.id);

	// Share the FD page with the caller
	*pg_store = o->o_fd;
	*perm_store = PTE_P|PTE_U|PTE_W | PTE_SHARED;
	return 0;
}
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	struct Fsret_read *ret = &ipc->readRet;
        ...
        off_t offset = 0;
        int r;
        char * buf = &ret->ret_buf[0];

         //memset(buf,97,PGSIZE);
	if (debug)
	...
	//
        //To Read the file , first find File* and the offset
        r= openfile_lookup ( envid,fileid, &ptr) ;

        if( r < 0 ) 
        {
             cprintf("MANI - unable to read file \n");
             return ( r ) ;
        }

         offset = ptr->o_fd->fd_offset;
         ...
         if(req_n > PGSIZE) 
         {
          ...
          //TODO : do we need to check if the user provided address is good ? 

         //cprintf("calling file_read function \n");
         r = file_read( ptr->o_file,(void*)buf,req_n,offset);

         //cprintf("MANI-file_read returned %d\n",r);
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if (debug)
		cprintf("serve_write %08x %08x %08x\n", envid, req->req_fileid, req->req_n);
         ...
         //use file_write t0 write to the file . it takes care of extending
         //the file size 
        r= openfile_lookup ( envid,fileid, &ptr) ;
        if( r < 0 ) 
        {
             cprintf("MANI - unable to write file \n");
             return ( r ) ;
        }
        ...
        if(req_n > PGSIZE)
        {
         ...
         r = file_write(ptr->o_file,buf,req_n,offset);
...
serve(void)
{
	...
	while (1) {
		perm = 0;
                //cprintf("MANI - FS waiting for messgae from client\n");
...
umain(void)
{
	...
	fs_test();
        ...
        cprintf("MANI :Server going to while loop\n");
	serve();
}
