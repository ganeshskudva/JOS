...
send_data(struct http_request *req, int fd)
{
	...
	//panic("send_data not implemented");
      ...
      bytes_to_read = filestat.st_size;
      if ((r = sys_page_alloc(0, UTEMP, PTE_P|PTE_U|PTE_W)) < 0)
	return r;

    while( bytes_to_read > 0 ) 
    {
       if ( (r = read(fd, UTEMP, bytes_to_read) )  < 0)
	return r;

       bytes_to_read -= r;
     
       //write the contents to socket
	if (write(req->sock, UTEMP, r) != r)
		return -1;
    }
...
send_file(struct http_request *req)
{
	...
	// set file_size to the size of the file
         ...
         if( fd > 0 ) 
         {
             ...
             if( result  != 0 )
             {
                 cprintf("mani - could not stat file %s\n",req->url);
                 send_error(req,404);
                 return -1;
             }
             else
             {
                   ...
                   {
                       cprintf("mani - given file is Dir stat file %s\n",req->url);
                      send_error(req,404);
                      return -1;
                   }

                    //set the filesize here 
                    file_size =  filestat.st_size;
             }
         }
         else
         { 
             cprintf("mani - could not open file %s\n",req->url);
             send_error(req,404);
             return -1;
         }
	// LAB 6: Your code here.
